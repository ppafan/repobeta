...
class Edit extends \Magento\Backend\App\Action
{
     ...
     * @see _isAllowed()
     */
    const ADMIN_RESOURCE = 'Acommerce_Address::subdistrict_edit';
    ...
    public function execute()
    {
        ...
        $subdistrict_id = (int) $this->getRequest()->getParam('subdistrict_id');
        $model = $this->_objectManager->create('Acommerce\Address\Model\Subdistrict');
        $model->setData([]);
        // Initial checking
        if ($subdistrict_id && $subdistrict_id > 0) {
            $model->load($subdistrict_id);
            if (!$model->getSubdistrictId()) {
        ...
        $this->_coreRegistry->register('acommerce_address_subdistrict', $model);
        ...
        $resultPage->addBreadcrumb(
            $id ? __('Edit Subdistrict') : __('New Subdistrict'),
            $id ? __('Edit Subdistrict') : __('New Subdistrict')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Subdistrict Manager'));
        $resultPage->getConfig()->getTitle()
            ->prepend($id ? 'Edit: '.$default_name.' ('.$id.')' : __('New Subdistrict'));
